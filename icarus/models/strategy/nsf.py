...
__all__ = [
    'calculate_greenness',
    'generate_graph',
    'generate_green_graph',
    'check_duplication',
    'sort_neighbors_not_duplicated',
    'calculate_green_path_neighbor_nodes',
    'calculate_green_path',
...
def calculate_greenness(node_id, time):
    filename = '/home/futureds/greenNDN/icarus/models/strategy/weather_data/node_' + str(node_id) + '.csv'
...
def generate_graph():
    graph_distance = fnss.parse_topology_zoo('/home/futureds/greenNDN/resources/topologies/NSFNET.graphml').to_undirected()
...
def check_duplication(graph, g_nodes, greenness):
...
def get_green_info():
    ...
    neighbor_gn_pair_dict = check_duplication(Graph, green_nodes_list, greenness)
    ...
    final_green_path_neighbor_nodes_dict = calculate_green_path_neighbor_nodes(neighbor_gn_pair_dict,
                                                                               final_green_path_gn_dict)
    ...
    # print final_green_path_gn_dict
    # print final_green_path_neighbor_nodes_dict

    return (green_nodes_list, neighbor_node_list, final_green_path_gn_dict, final_green_path_neighbor_nodes_dict)
...
def set_green_path(green_path_dict, associated_green_path_dict, allpath):
    green_path  = copy.deepcopy(allpath)
    associate_green_path = copy.deepcopy(allpath)

    for key, value in green_path_dict.iteritems():
        for subkey, subval in value.items():
            green_path[key][subkey] = copy.deepcopy(subval)

    for key, value in associated_green_path_dict.iteritems():
        for subkey, subval in value.items():
            associate_green_path[key][subkey] = copy.deepcopy(subval)


    # print green_path
    # print associate_green_path

    return (green_path, associate_green_path)



def get_green_path(green_path, receiver, source):
    print receiver, source
    # print green_path[receiver]
    # print green_path[receiver][source]
    # return green_path[receiver][source]

    return green_path[4][21]
